...
def get_array_shape(arr):
    ...
    if len(x) == 3:
        return "sor: " + str(x[1]) + ", oszlop: " + str(x[2]) + ", melyseg: " + str(x[0])
...
# %%
def eval_classification(options, arr):
    return options[np.argmax(arr)]
...
# %%
def replace_by_value(arr, threshold):
    arr[arr < threshold] = -1
    arr[arr >= threshold] = 1
    return arr
...
# %%
def list_days(aDate, bDate):
    x = np.arange(start= np.datetime64(aDate), stop= np.datetime64(bDate), step= np.timedelta64(1, 'D'))
    return x
...
# %%
def sec_from_1970():
    diff = np.datetime64("now") - np.datetime64("1970-01-01 00:02:00")

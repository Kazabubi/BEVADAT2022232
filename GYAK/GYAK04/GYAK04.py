...
# %%
def get_column(df : pd.DataFrame, spec : str):
    df_out = df.copy()
    return df_out[spec]
#get_column(df_stats, "country")
...
def get_top_two(df : pd.DataFrame):
    df_out = df.copy()
    df_out.sort_values(by=["area"], inplace=True, ascending=False)
    return df_out.head(2)
...
def population_density(df : pd.DataFrame):
    df_out = df
    density = df_out["population"] / df_out["area"]
    df_out["density"] = density
    return df_out
